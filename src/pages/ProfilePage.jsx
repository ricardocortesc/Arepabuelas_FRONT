...
const ProfilePage = () => {
  const { currentUser, purchaseHistory, fetchOrderHistory } = useApp();

  // Cargar el historial cuando la página se monta
  useEffect(() => {
    if (currentUser) {
      fetchOrderHistory(currentUser.id);
    }
  }, [currentUser, fetchOrderHistory]);
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div className="flex items-center space-x-4 mb-8">
        <img src={currentUser.photoUrl || 'https://placehold.co/100x100/888888/FFFFFF?text=User'} alt={currentUser.name} className="h-24 w-24 rounded-full shadow-lg" />
        <div>
                ...
                <div key={order.id} className="border p-4 rounded-lg">
                  ...
                  <ul className="list-disc list-inside text-sm text-gray-600 mb-2">
                    {order.items.map((item, index) => (
                      <li key={`${item.id}-${index}`}>{item.name || 'Producto'} (x{item.quantity})</li>
                    ))}
                  </ul>
                  {order.discount > 0 && (
